...
def train_model(model, train_loader, val_loader, num_epochs=50, lr=0.001):
    ...
    for epoch in range(num_epochs):
        ...
        try:
            ...
            print(f"Warning: Could not write to epoch.txt. Error: {e}")
...
def main():
    ''' 1. Calculate the mean and standard deviation for normalization. '''
    ...
    train_stats = SimpsonsDataset(TRAIN_PATH, transform=stats_transform, is_train=True)
    if(len(train_stats) == 0):
        print(f"Error: No images found in '{TRAIN_PATH}'. Please check the path.")
        return

    train_mean, train_std = calculate_mean_std(train_stats, BATCH_SIZE, num_workers=8)
    print(f"Dataset Mean: {train_mean}")
    print(f"Dataset Std: {train_std}\n")


    ''' 2. Define data augmentations and transformations. '''
    train_transform = transforms.Compose([
        ...
        transforms.RandomApply([transforms.RandomAffine(degrees=(-70, 70), translate=(0.1, 0.3), scale=(0.5, 0.75))], p=0.2),
        ...
        transforms.RandomApply([transforms.RandomSolarize(threshold=0.5)], p=0.2),
        ...
        transforms.RandomApply([AddPoissonNoise(lam=0.15)], p=0.2),
        transforms.RandomApply([AddSpeckleNoise(noise_level=0.15)], p=0.2),
    ...
    val_loader = DataLoader(val_subset, BATCH_SIZE, shuffle=False, num_workers=8, pin_memory=True)

    ''' 4. Initialize model, train, and evaluate. '''
    print("Initializing model...")
    model = EfficientNetClassifier(num_classes=50).to(device)
    ...
    train_losses, val_losses, train_accs, val_accs = train_model(
        model, train_loader, val_loader, num_epochs=50, lr=0.003
    )
